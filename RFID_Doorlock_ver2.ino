...
void setup() {                                                            
  ...
  char *hello = TCP_Packet_Char("/addtest.php?nfc=setup_ok");       //TCP연결 "setup_ok" 메시지를 
...
char* TCP_Packet_Char(char *arg1) //HTTP패킷 전송 함수(Character 형)
{
  temp = String(String("GET ") + arg1 +
         String("\r\nHost: 192.168.0.22\r\nConnection: close\r\n\r\n"));
  temp.toCharArray(buffer, temp.length());
  return buffer;
}

char* TCP_Packet_Byte(byte argc[]) //HTTP패킷 전송 함수(Byte 형)
{
